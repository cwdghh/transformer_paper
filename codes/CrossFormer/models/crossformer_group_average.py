...
class CrossFormerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, group_size=7, lsda_flag=0,
        ...
        if self.lsda_flag == 0 or input_resolution[0] == group_size[0]: # 0 for SDA
            self.attn = Attention(
                dim, group_size=to_2tuple(self.group_size), num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
                position_bias=True)
        else:
            self.attn = Mutual_Attention(
                dim, group_size=to_2tuple(self.group_size), num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
                position_bias=False)
    ...
    def forward(self, x):
        ...
        # group embeddings
        G = self.group_size
        if self.lsda_flag == 0 or self.input_resolution[0] == self.group_size[0]: # 0 for SDA
            ...
            y = x.reshape(B, G, H // G, G, W // G, C).permute(0, 2, 4, 1, 3, 5)
