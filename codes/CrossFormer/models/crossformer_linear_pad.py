...
class CrossFormerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # compute attention mask and pad the input x
        self.attn_mask = torch.ones((H, W)).cuda()
        pad_num = (int(np.ceil(H / G))) * G - H # how much need to be padded
        # four directions: left, right, top, down
        if pad_num % 2 == 0:
            pad_size = [pad_num // 2] * 4
        else:
            pad_size = [(pad_num - 1) // 2, (pad_num + 1) // 2] * 2
        # generate mask
        self.attn_mask = F.pad(self.attn_mask, pad_size)
        self.attn_mask = self.attn_mask.reshape((H + pad_num) // G, G, (W + pad_num) // G, G).permute(0, 2, 1 ,3).flatten(-2).flatten(0, 1)
        self.attn_mask = self.attn_mask.unsqueeze(-1) * self.attn_mask.unsqueeze(-2)
        self.attn_mask = (1 - self.attn_mask) * -10000

        # group embeddings
        x = F.pad(x, [0, 0] + pad_size)
        if self.lsda_flag == 0: # 0 for SDA
            x = x.reshape(B, (H + pad_num) // G, G, (W + pad_num) // G, G, C).permute(0, 1, 3, 2, 4, 5)
        else: # 1 for LDA
            x = x.reshape(B, G, (H + pad_num) // G, G, (W + pad_num) // G, C).permute(0, 2, 4, 1, 3, 5)
        ...
        # ungroup embeddings
        x = x.reshape(B, (H + pad_num) // G, (W + pad_num) // G, G, G, C)
        if self.lsda_flag == 0:
            x = x.permute(0, 1, 3, 2, 4, 5).reshape(B, H + pad_num, W + pad_num, C)
        else:
            x = x.permute(0, 3, 1, 4, 2, 5).reshape(B, H + pad_num, W + pad_num, C)
        x = x[:, pad_num // 2: pad_num // 2 + H, pad_num // 2: pad_num // 2 + W, :]
        x = x.reshape(B, H * W, C)
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, group_size,
        ...
        for i in range(depth):
            ...
            self.blocks.append(CrossFormerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, group_size=group_size[i],
                                 lsda_flag=lsda_flag,
                                 mlp_ratio=mlp_ratio,
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, num_classes=1000,
        ...
        group_size = [[] for i in range(len(depths))]

        cnt = 0
        for i_stage in range(self.num_layers):
            # assume width and height to be the same at this time
            cur_resolution = patches_resolution[0] // 2 ** i_stage
            for i_block in range(depths[i_stage]):
                gz = cur_resolution * group_fraction[cnt]
                if gz > cur_resolution // 2:
                    gz = cur_resolution if gz > cur_resolution * 3 / 4 else cur_resolution // 2
                group_size[i_stage].append(max(4, int(np.ceil(gz))))
                cnt += 1
